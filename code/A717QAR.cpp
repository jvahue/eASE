...
void A717Qar::InitIoi()
{
  ...
  // Open the status IOI
  OpenIoi( QAR_A717_STATUS_NAME, &m_moduleStatusIoi,
          (void*)&m_qarModStatus, sizeof(m_qarModStatus),
...
void A717Qar::UpdateIoi()
{
  ...
  // Is it time to send a SF something?
  // Output the next SF(if QAR active) and the status msg.
  if ( TRUE == bSendNow || m_crntTick == eTICKS_PER_SEC )  //send data at 1Hz
  {
...
bool A717Subframe::UpdateBuffer( UINT32 offset, UINT8* pBuffer, UINT32 byteCnt )
{
    ...
    // custom barker code. Dont't use the default barker until the tester has called 
